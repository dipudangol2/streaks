...
const HabitForm = ({ onSuccess }: { onSuccess?: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post<CreateHabitResponse, CreateHabitPayload>(
        "/habits",
        {
          title,
          description,
      ...
      toast.error(`Error during habit creation: ${(error as Error).message}`);
      return;
    }
